"""
HiveMind Client — swarm intelligence sync with Coin Screener server.
Pull crowd lessons, push local lessons (signed), auto-evolve thresholds.
"""
import json
import os
import logging
...
class HiveMindClient:
    def __init__(self, agent_id: str = None, server_url: str = None):
        ...
        if not self.enabled:
            logger.info("[HiveMind] No server URL — swarm disabled. Set HIVEMIND_URL + HIVEMIND_AGENT_ID in .env")
    ...
    def push_lesson(self, rule: str, tags: list, regime: str, signal: str,
                    result: str, pnl_pct: float, confidence: float) -> bool:
        body = {
            "agent_id": self.agent_id,
    ...
    def push_performance_event(self, symbol: str, regime: str, signal: str,
        ...
        body = self._sign_body(body)
        resp = self._request("performance/push", "POST", body)
        return resp is not None
