"""
Agent Memory — bounded persistent memory (max 2,200 chars ~800 tokens).
...
class AgentMemory:
    """Bounded persistent memory stored as entries separated by § in data/MEMORY.md."""

    def __init__(self, memory_path: str = MEMORY_PATH, max_chars: int = MAX_CHARS):
    ...
    def add_entry(self, text: str) -> bool:
        """Add a memory entry. Returns False if >80% full (needs consolidation first)."""
        text = text.strip()
        if not text:
            return False
    ...
    def consolidate(self) -> str:
        ...
        # Consolidation prompt for LLM — here we provide a simple heuristic merge
        combined = " | ".join(entries)
        if len(combined) > self.max_chars * 0.6:
            # Truncate by merging old entries with a summary marker
